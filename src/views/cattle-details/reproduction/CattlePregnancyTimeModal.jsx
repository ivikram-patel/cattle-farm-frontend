...
const CattlePregnancyTimeModal = (props) => {
    const [formData, setFormData] = useState({
        id: props.cattleId,
        cattle_tag: '',
        cattle_pregnancy_time: dayjs(),
        note: '',
    });

    const fetchData = async () => {
        try {
            const response = await axiosInstance.get(`api/cattle-pregnancy-detail/${props.cattleId}`);

            setFormData({
                ...formData,
                cattle_tag: response.data.tag_no,
                cattle_pregnancy_time: response.data.time,
    ...
    const submitForm = async () => {
        ...
        let endPoint = `api/submit-cattle-pregnancy`;
                            ...
                            <LocalizationProvider dateAdapter={AdapterDayjs} fullWidth>
                                <Stack spacing={3}>
                                    <DesktopDatePicker
                                        disableFuture
                                        label="Time"
                                        value={dayjs.utc(formData.cattle_pregnancy_time)}
                                        // minDate={new Date('01-01-2014')}
                                        format="DD-MM-YYYY"
                                        defaultValue={dayjs()}
                                        onChange={(newValue) => {
                                            setFormData({ ...formData, cattle_pregnancy_time: newValue.tz('Asia/Kolkata').toDate() })
                                        }}
                                        renderInput={(params) => (
                                            <TextField
                                                {...params}
...
export default CattlePregnancyTimeModal